...
export function analyzeProject(input: AnalysisInput): AnalysisReport | null {
  ...
  if (mode === 'routes') {
    ...
    enrichRouteMigration(input, routes, new AngularProjectAnalyzer().analyze({ ...input, mode: 'structural', migrationRules: [], componentMappings: {} }));
  ...
  const routes = mode === 'complete' ? new AngularRouteAnalyzer().analyze(input) : null;
  if (routes?.totalRoutes) enrichRouteMigration(input, routes, structural);
  ...
  return {
    schemaVersion: routes?.totalRoutes ? 4 : 3, ...(routes?.totalRoutes ? { routes } : {}), analysisId: randomUUID(), projectId: input.projectId,
    projectName: input.projectName, analyzedAt: new Date().toISOString(), analyzedFiles: input.files.length,
    mode, structural, ui, warnings: [...new Set([...(structural?.warnings ?? []), ...(ui?.warnings ?? []), ...(routes?.totalRoutes ? routes.warnings : [])])],
